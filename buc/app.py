...
def dashboard():
    ...
    #     os.system('"/mnt/c/Program Files (x86)/google/Chrome/Application/chrome.exe" http://192.168.69.112:8080/guacamole/#/client/a2FpX3dpbjEwAGMAZGVmYXVsdA==')
    return render_template('dashboard.html', name=current_user.username, w="3", l="5", a="2")
    # return render_template('winVM.html')

@app.route('/dashboard/windows',  methods=['GET', 'POST'])
@login_required
def windowsvm_show():
    return render_template('dashboard.html', w="3", l="0", a="0")
@app.route('/dashboard/linux',  methods=['GET', 'POST'])
@login_required
def linuxvm_show():
    return render_template('dashboard.html', w="0", l="5", a="0")
@app.route('/dashboard/app',  methods=['GET', 'POST'])
@login_required
def appvm_show():
    return render_template('dashboard.html', w="0", l="0", a="2")
